 ...
 */
export const FloatingNumbersOverlay = observer(
  function FloatingNumbersOverlay() {
    const activityStore = useActivityStore();

    if (activityStore.floatingNumbers.length === 0) return null;

    return createPortal(
      <div className="pointer-events-none fixed top-32 left-64 z-50">
        {activityStore.floatingNumbers.map((fn, index) => {
          const isPositive = fn.value > 0;
          const prefix = isPositive ? '+' : '';
          const displayValue =
            Math.abs(fn.value) >= 1 ? fn.value.toFixed(0) : fn.value.toFixed(1);
