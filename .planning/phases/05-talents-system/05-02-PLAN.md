...
must_haves:
  truths:
    ...
    - "10 talents exist in the pool with rarity distribution"
  ...
  artifacts:
    ...
    - path: "src/data/talents.ts"
      provides: "Talent pool data (10 talents)"
      exports: ["TALENTS"]
...
<task type="auto">
  <name>Task 2: Create talent seed data (resource and capacity effects only)</name>
  <files>src/data/talents.ts</files>
  <action>
Create new file with 10 talents targeting ONLY resources and capacities (MVP scope - skill/activity modifiers deferred to future phase).

IMPORTANT: Only use these effect targets for MVP:
- `target: 'resource'` with `type: 'percentage'` - modifies drain/recovery rates
- `target: 'capacity'` with `type: 'flat'` - adds flat bonus to capacity

Do NOT use `target: 'skill'` or `target: 'activity'` - these require additional store integration not in this phase.
...
```
  ...
  <verify>`npx tsc --noEmit` passes</verify>
  <done>10 talents exported from talents.ts with resource and capacity effects only (no skill/activity modifiers)</done>
</task>
...
- All talents use only resource or capacity effects (no skill/activity targets)
