...
|---------|---------|---------|-------------|
| DaisyUI | 5.x | Rating component for star display | `rating` class with `mask mask-star-2` for read-only stars, semantic colors (bg-success/warning/error) |
...
### Pattern 5: DaisyUI Rating Component for Star Display
**What:** Use DaisyUI's `rating` component with read-only `div` elements (not radio inputs) for displaying difficulty stars. Apply background colors to filled stars based on difficulty level.
**When to use:** Display-only star ratings with color coding and proper styling.
**Example:**
```typescript
// Source: DaisyUI rating component docs - read-only variant
// https://daisyui.com/components/rating/

// Map difficulty to color class
function getDifficultyColorClass(stars: number): string {
  if (stars <= 2) return 'bg-success'; // 1-2 stars = green (easy)
  if (stars <= 3.5) return 'bg-warning'; // 3 stars = yellow (medium)
  return 'bg-error'; // 4-5 stars = red (hard)
}

// Read-only rating display using DaisyUI
  ...
  return (
    <div className="rating rating-sm">
      {[1, 2, 3, 4, 5].map((star) => (
        <div
          key={star}
          className={`mask mask-star-2 ${star <= filled ? colorClass : 'bg-base-300'}`}
          aria-label={`${star} star`}
          aria-current={star === filled ? 'true' : undefined}
        />
      ))}
    </div>
  );
...
- `rating-xs`, `rating-sm`, `rating-md`, `rating-lg`, `rating-xl` - Size modifiers
- `mask mask-star-2` - Star shape (filled star appearance)
- `bg-success`, `bg-warning`, `bg-error` - Semantic colors for difficulty
- `bg-base-300` - Muted color for unfilled stars
...
### Star Display Component (DaisyUI Rating)
```typescript
// Source: DaisyUI rating component - read-only variant
// https://daisyui.com/components/rating/
function DifficultyStars({
  ...
  // Color based on difficulty (for filled stars)
  const colorClass = difficulty <= 2
    ? 'bg-success'      // 1-2 stars = green (easy)
    : difficulty <= 3.5
      ? 'bg-warning'    // 3 stars = yellow (medium)
      : 'bg-error';     // 4-5 stars = red (hard)

  return (
    <div>
      {/* DaisyUI rating component - read-only using divs */}
      <div
        ref={triggerRef}
        className={`rating rating-${size} cursor-help`}
