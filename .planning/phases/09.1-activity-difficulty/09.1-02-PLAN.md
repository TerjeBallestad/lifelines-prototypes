...
<task type="auto">
  ...
  <action>
Create `src/components/DifficultyStars.tsx` using DaisyUI's rating component:

1. Import React, useId, useRef from 'react'
2. Import observer from 'mobx-react-lite'
3. Import DifficultyBreakdown from '../types/difficulty'

4. Create color mapping function for filled stars:
   - 1-2 stars: 'bg-success' (green - easy)
   - 2.5-3.5 stars: 'bg-warning' (yellow - medium)
   - 4-5 stars: 'bg-error' (red - hard)
   - Unfilled stars: 'bg-base-300' (muted)
...
6. Implement star display using DaisyUI rating component (read-only pattern):
   ```tsx
   <div className={`rating rating-${size}`}>
     {[1, 2, 3, 4, 5].map((star) => (
       <div
         key={star}
         className={`mask mask-star-2 ${star <= Math.round(difficulty) ? colorClass : 'bg-base-300'}`}
         aria-label={`${star} star`}
       />
     ))}
   </div>
   ```
   - Use `div` elements (not inputs) for read-only display
   - `mask mask-star-2` for filled star shape
   - Apply color to filled stars, muted to unfilled

7. Implement native Popover API tooltip (matching ActionResourcesSection pattern):
   - Wrap rating in a div with ref for popover positioning
...
TypeScript compiles: `npm run build` or `npx tsc --noEmit`
  ...
  <done>
DifficultyStars component uses DaisyUI `rating` container with `mask mask-star-2` divs for read-only star display. Filled stars use semantic colors (bg-success/warning/error), unfilled use bg-base-300. Popover breakdown on hover matches ActionResourcesSection pattern.
  </done>
</task>
...
- Stars use DaisyUI rating component (`rating` + `mask mask-star-2`) for consistent styling
- Stars are color-coded (bg-success green easy, bg-warning yellow medium, bg-error red hard)
...
- UI matches established patterns (native Popover API, DaisyUI rating component)
</success_criteria>
